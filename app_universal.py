# MAINTAIN-AI ULTIMATE - FIXED RAG COLORS - NO CSS INDENT ERROR
import pandas as pd
...
st.set_page_config(page_title="MAINTAIN-AI ULTIMATE RAG FIXED", layout="wide")
...
st.title(f"🏭 MAINTAIN-AI | {client_info['client']} | UNIVERSAL RAG FIXED")
st.markdown("**Flow Station | Rig | Gas Plant | Power Plant - RAG Colors Fixed: 🔴 Red | 🟡 Amber | 🟢 Green + Grey/Green Tick**")
...
if data_source == "Upload Company Excel":
    ...
    if not xlsx_names:
        st.error("No Excel in folder. Add Sample-Maintenance-Log-20-Assets-FIXED.xlsx or switch to Upload")
        st.stop()
...
for asset_type, default_days in profile['intervals'].items():
    intervals[asset_type] = st.sidebar.number_input(f"{asset_type}", min_value=7, max_value=730, value=default_days, key=f"ragfix_{plant_type}_{asset_type}")
...
def calc_ai_risk(row):
    ...
    if overdue > 0:
        return min(0.95 + (overdue/100), 1.0)
    else:
        return max(0.1, min(0.8, days_since/interval*0.8))
...
# Session state for serviced assets (fixes cloud mode where file can't be written)
...
for asset_tag, service_date in st.session_state.serviced_assets.items():
    if asset_tag in latest['Asset Tag'].values:
        ...
        latest.loc[latest['Asset Tag']==asset_tag, 'Days Overdue By'] = (datetime.now() - service_date).days - latest.loc[latest['Asset Tag']==asset_tag, 'Interval_Days'].iloc[0]
        latest.loc[latest['Asset Tag']==asset_tag, 'Rule_Status'] = np.where(latest.loc[latest['Asset Tag']==asset_tag, 'Days Overdue By'] > 0, 'OVERDUE', 'OK')
        latest.loc[latest['Asset Tag']==asset_tag, 'RAG_Status'] = latest.loc[latest['Asset Tag']==asset_tag, 'Days Overdue By'].apply(get_rag_status)
        latest.loc[latest['Asset Tag']==asset_tag, 'RAG_Emoji'] = latest.loc[latest['Asset Tag']==asset_tag, 'Days Overdue By'].apply(get_rag_emoji)
...
def create_pdf(row, plant_type, profile, client_name, plant_location, prepared_by, logo_text):
    try:
        ...
        story.append(Paragraph(f"<b>Date:</b> {datetime.now().strftime('%d/%m/%Y')} | <b>WO:</b> WO-{row['Asset Tag']}-{datetime.now().strftime('%Y%m%d')} | <b>RAG:</b> {row['RAG_Status']}", styles['Normal']))
        story.append(Spacer(1, 12))
        data = [
...
# RAG Filter FIXED
st.subheader("RAG Filter:")
rag_filter = st.radio("RAG Filter:", ["All", "🔴 Red - OVERDUE", "🟡 Amber - Due 7 days", "🟢 Green - OK"], horizontal=True, label_visibility="collapsed")
...
st.dataframe(rag_df[['Asset Tag','Equipment','Days Overdue By','RAG_Status','Next Service Due','Is_Critical','Location']].sort_values('Days Overdue By', ascending=False), use_container_width=True, height=250)

st.header(f"📝 Work Orders + PDF + Grey/Green Tick - {plant_type} - RAG FIXED")
st.caption("🔴 Red = OVERDUE (Not Serviced - Grey) | 🟡 Amber = Due in 7 days | 🟢 Green = OK (Serviced - Green Tick ✅)")
...
for _, row in latest.sort_values('Days Overdue By', ascending=False).iterrows():
    ...
    with st.container(border=True):
        ...
        with col_a:
            ...
            # FIXED: Show RAG emoji + Green tick
            if is_green:
            ...
            if is_green:
                ...
                st.error(f"🔴 OVERDUE {int(row['Days Overdue By'])}d - NUPRC {profile['compliance']} + AI {row.get('AI_Risk',0):.0%} risk")
        with col_b:
        ...
        with col_d:
            # FIXED: Button colors - Grey for Not Serviced, Green for Serviced
            if is_overdue or is_amber:
                # Overdue or Amber = Not Serviced = Grey button
                ...
                # Green = Serviced = Green tick button
                st.button(f"✅ Serviced - GREEN", key=f"serv_green_{row['Asset Tag']}_{plant_type}", use_container_width=True, disabled=True)
                if st.button(f"↩️ Undo (Mark Overdue)", key=f"undo_{row['Asset Tag']}_{plant_type}", use_container_width=True):
                    if row['Asset Tag'] in st.session_state.serviced_assets:
                        del st.session_state.serviced_assets[row['Asset Tag']]
                    st.rerun()

st.success(f"✅ {plant_type} RAG FIXED: 🔴 {len(latest[latest['Days Overdue By']>0])} Red OVERDUE | 🟡 {len(latest[(latest['Days Overdue By'] >= -7) & (latest['Days Overdue By'] <=0)])} Amber Due 7d | 🟢 {len(latest[latest['Days Overdue By']<-7])} Green OK")
