# app_final_CLEAN.py - CLEAN FINAL: Upload + Refresh + PDF + Grey/Green Tick
import pandas as pd
...
# CSS for grey vs green serviced button
st.markdown("""
<style>
/* Green tick for serviced (OK) */
...
/* Grey tick for not serviced (OVERDUE) */
...
st.title("🏭 MAINTAIN-AI | UNIVERSAL + PDF Work Order (FINAL)")
st.markdown("**Flow Station | Rig | Gas Plant | Power Plant → Auto intervals 21/30/180 + PDF + Grey/Green Tick**")
...
if data_source == "Upload Company Excel":
    ...
    if uploaded_file is None:
        st.info("👈 Sidebar → Select 'Upload Company Excel' → Drag your company Excel file. Template: Asset Tag, Equipment, Last Service, Location, Failure, Date")
...
st.header(f"📝 Work Orders + PDF + Status Tick - {plant_type}")
st.caption("⬜ Grey = Not Serviced (OVERDUE) | ✅ Green = Serviced (OK) | Click to mark serviced TODAY")
    ...
    with st.container(border=True):
        ...
        with col_d:
            ...
            if row['Rule_Status'] == 'OVERDUE':
                # Grey tick - not serviced
                btn_label = "⬜ Not Serviced"
                btn_type = "secondary"
                help_text = f"Overdue by {int(row['Days Overdue By'])}d - Click to mark as serviced TODAY"
            else:
                # Green tick - serviced and OK
                btn_label = "✅ Serviced"
                btn_type = "primary"
                help_text = f"Serviced {int(row['Days Since Service'])}d ago - OK till {row['Next Service Due'].strftime('%d/%m/%Y')}"
            
            if st.button(btn_label, key=f"serv_{row['Asset Tag']}_{plant_type}", use_container_width=True, type=btn_type, help=help_text):
                if full_path:
                    try:
...
st.success(f"✅ {plant_type} Active: {len(latest)} assets | Overdue {len(latest[latest['Rule_Status']=='OVERDUE'])} (⬜ Grey) | OK {len(latest[latest['Rule_Status']=='OK'])} (✅ Green) | File: {file_name_label}")
            "Equipment": ["Sulzer Crude Pump", "Caterpillar 3512B Generator", "Separator Vessel"],
            "Date": ["01/09/2026", "02/09/2026", "03/09/2026"],
            "Last Service": ["15/08/2026", "01/08/2026", "01/03/2026"],
            "Failure": ["Bearing hot", "Low voltage", "Level issue"],
