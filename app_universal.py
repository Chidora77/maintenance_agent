# app_FINAL_ULTIMATE.py - EVERYTHING: Login + Logout + Grey/Green + Hybrid Decision + Upload + Refresh + PDF + No Install Text + No Pricing
import pandas as pd
...
# ============ LICENSE SYSTEM - NO PRICING DISPLAY ============
VALID_LICENSES = {
...
# ============ PAGE CONFIG + CSS GREY/GREEN ============
st.set_page_config(page_title="MAINTAIN-AI ULTIMATE", layout="wide", page_icon="🏭")
st.markdown("""
<style>
...
# Hybrid Decision Section - RESTORED
...
for _, row in latest.sort_values('Days Overdue By', ascending=False).head(10).iterrows():
    with st.container(border=True):
        ...
        with col1:
            ...
            st.markdown(f"**{emoji} {row['Asset Tag']} - {row['Equipment']}**")
        ...
        with col_d:
            ...
            if st.button(btn_label, key=f"serv_{row['Asset Tag']}_{plant_type}", use_container_width=True, type=btn_type):
                if full_path:
                    ...
                    buf.seek(0)
                    st.download_button("⬇️ Download Updated Excel", buf, file_name=f"Updated_{row['Asset Tag']}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"dl_{row['Asset Tag']}_{plant_type}")
