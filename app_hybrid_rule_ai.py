...
st.sidebar.header("🔄 Refresh - FIXED")
if st.sidebar.button("🔄 Manual Refresh (No Logout)", use_container_width=True, key="manual_refresh_top_btn"):
    st.rerun()

auto_enabled = st.sidebar.toggle("Auto-refresh 10 mins", value=False, key="auto_refresh_toggle_unique")
interval_mins = st.sidebar.slider("Refresh interval (mins)", 1, 60, 10, key="refresh_interval_slider_unique")
if auto_enabled:
    import time
    if 'last_refresh' not in st.session_state:
        st.session_state.last_refresh = time.time()
    elapsed = time.time() - st.session_state.last_refresh
    remaining = int(interval_mins*60 - elapsed)
    if elapsed > interval_mins * 60:
        st.session_state.last_refresh = time.time()
        st.rerun()
    st.sidebar.caption(f"⏱️ Next auto-refresh in {max(0,remaining)}s - Auth kept ✅")
else:
