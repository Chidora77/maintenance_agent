        ...
        with col_d:
            if is_overdue or is_amber:
                if st.button(f"❌ Not Serviced", key=f"serv_{row['Asset Tag']}_{plant_type}_final_unique", use_container_width=True):
                    # FIXED: Keep REAL TIME (not 00:00:00) but ensure correct date
                    # Use local time with actual hours/minutes/seconds
                    now_dt = datetime.now()  # Real time e.g. 24/09/2026 10:12:42
                    now_for_file = pd.to_datetime(now_dt)  # Full datetime with real time
                    ...
                    if full_path:
                        try:
                            ...
                            # FIXED: Write full datetime with REAL TIME (10:12:42 not 00:00:00)
                            df_full.loc[mask, 'Last Service'] = now_for_file
                            if 'Date' in df_full.columns:
                                df_full.loc[mask, 'Date'] = now_for_file
                            # Save with datetime format preserved
