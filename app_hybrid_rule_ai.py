...
# REFRESH - SEPARATED MANUAL + AUTO - NO MIXING
st.sidebar.markdown("---")
st.sidebar.subheader("🔄 Refresh")

# MANUAL - INDEPENDENT - ALWAYS CLICKABLE
if st.sidebar.button("🔄 Manual Refresh", use_container_width=True, key="manual_refresh_separate_unique_v3", type="primary", help="Manual refresh - stays logged in"):
    st.toast("Manual refresh...", icon="🔄")
    try:
        st.cache_data.clear()
    except:
        pass
...
st.sidebar.caption("Manual: click to refresh instantly ✅")
st.sidebar.markdown("---")

# AUTO - COMPLETELY SEPARATE - DOES NOT INTERFERE WITH MANUAL
st.sidebar.markdown("**Auto-refresh (optional)**")
auto_enable = st.sidebar.checkbox("Enable auto-refresh", value=False, key="auto_enable_separate_v3_unique")
auto_interval = st.sidebar.number_input("Interval (mins)", min_value=1, max_value=60, value=10, key="auto_interval_separate_v3_unique")

if auto_enable:
    import time
    if 'auto_last_refresh' not in st.session_state:
        st.session_state.auto_last_refresh = time.time()
    elapsed_auto = time.time() - st.session_state.auto_last_refresh
    remaining_auto = max(0, int(auto_interval*60 - elapsed_auto))
    st.sidebar.caption(f"Auto in {remaining_auto}s - stays logged in ✅")
    if elapsed_auto > auto_interval * 60:
        st.session_state.auto_last_refresh = time.time()
        try:
            st.cache_data.clear()
        except:
            pass
        st.toast(f"Auto-refreshing after {auto_interval} mins", icon="⏱️")
        st.rerun()
else:
    st.sidebar.caption("Auto-refresh OFF - manual only")

# DOWNLOAD BUTTON IN SIDEBAR - QUICK ACCESS
st.sidebar.markdown("---")
st.sidebar.subheader("📥 Download Excel")
try:
    import glob as glob_dl
    dl_files = [f for f in glob_dl.glob(os.path.join(SCRIPT_DIR, "*.xlsx")) if not os.path.basename(f).startswith("~$")]
    if dl_files:
        dl_path_sb = dl_files[0]
        with open(dl_path_sb, 'rb') as f:
            sb_bytes = f.read()
        st.sidebar.download_button(
            label=f"📥 Download {os.path.basename(dl_path_sb)}",
            data=sb_bytes,
            file_name=f"UPDATED_{os.path.basename(dl_path_sb)}",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="download_excel_sidebar_unique",
            use_container_width=True
        )
    else:
        st.sidebar.caption("No file in folder")
except Exception as e:
    st.sidebar.caption(f"Download error: {e}")

st.sidebar.markdown("---")

st.title(f"MAINTAIN-AI | {client_info['client']}")

# SHOW LAST SPREADSHEET UPDATE VERIFICATION AT TOP + DOWNLOAD
...
# DOWNLOAD UPDATED SPREADSHEET SECTION - ALWAYS VISIBLE AT TOP
st.markdown("---")
st.subheader("📥 Download Updated Spreadsheet")
st.caption("After you tick service button, file is updated on server. Download it here to see new date.")

# Try to find the current Excel file to download
try:
    import os
    from io import BytesIO
    SCRIPT_DIR_DL = os.path.dirname(os.path.abspath(__file__))
    # Get latest file path from session or folder
    dl_path = None
    dl_name = None
    if 'last_file_updates' in st.session_state and st.session_state.last_file_updates:
        # Use last updated file
        last_info = list(st.session_state.last_file_updates.values())[-1]
        dl_path = last_info.get('path')
        dl_name = last_info.get('file', 'updated_maintenance.xlsx')
    
    # Fallback to current selected file
    if not dl_path or not os.path.exists(dl_path):
        import glob
        xlsx_files_dl = [f for f in glob.glob(os.path.join(SCRIPT_DIR_DL, "*.xlsx")) if not os.path.basename(f).startswith("~$")]
        if xlsx_files_dl:
            dl_path = xlsx_files_dl[0]
            dl_name = os.path.basename(dl_path)
    
    if dl_path and os.path.exists(dl_path):
        # Read file and create download buffer
        with open(dl_path, 'rb') as f:
            file_bytes = f.read()
        mtime = os.path.getmtime(dl_path)
        mtime_str = datetime.fromtimestamp(mtime).strftime("%d/%m/%Y %H:%M:%S")
        st.success(f"Current file: {dl_name} | Last modified: {mtime_str} | Size: {len(file_bytes)} bytes")
        st.download_button(
            label=f"📥 Download {dl_name} (Updated)",
            data=file_bytes,
            file_name=f"UPDATED_{dl_name}",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="download_updated_excel_main_unique",
            use_container_width=True,
            type="primary"
        )
        # Also show preview of last serviced dates
        try:
            df_preview = pd.read_excel(dl_path, engine='openpyxl')
            if 'Asset Tag' in df_preview.columns and 'Last Service' in df_preview.columns:
                st.caption("Preview - Last Service dates in file:")
                st.dataframe(df_preview[['Asset Tag','Equipment','Last Service']].sort_values('Last Service', ascending=False).head(5), use_container_width=True, height=200)
        except Exception as e:
            st.caption(f"Preview failed: {e}")
    else:
        st.warning("No Excel file found on server to download. If you used 'Upload Company Excel' mode, download below.")
        # For upload mode, create updated file from df if available
        try:
            # This will be defined later, but we try to create from current df
...
if 'original_service_dates' not in st.session_state:
    st.session_state.original_service_dates = {}
...
for asset_tag, service_date in st.session_state.serviced_assets.items():
    if asset_tag in latest['Asset Tag'].values:
        ...
        latest.loc[latest['Asset Tag']==asset_tag, 'AI_Status'] = "LOW"
        # FIXED: Also update Hybrid_Decision so green assets don't show OVERDUE
        new_emoji = get_rag_emoji(overdue)
        new_due = (service_date + pd.to_timedelta(latest.loc[latest['Asset Tag']==asset_tag, 'Interval_Days'].iloc[0], unit='D')).strftime('%d/%m')
        if overdue > 0:
            latest.loc[latest['Asset Tag']==asset_tag, 'Hybrid_Decision'] = f"{new_emoji} OVERDUE - {profile['compliance']} + AI 5% risk"
        else:
            latest.loc[latest['Asset Tag']==asset_tag, 'Hybrid_Decision'] = f"{new_emoji} OK - Next {new_due} - AI 5% - Green OK" 
...
st.caption("Bar color = RAG status: 🔴 Red = 100% overdue, 🟡 Amber = due 7d, 🟢 Green = OK")
for _, row in latest.sort_values('Days Overdue By', ascending=False).head(15).iterrows():
    with st.container(border=True):
        ...
        with col1:
            ...
            # FIXED: RAG colored bar - Red 100% = red line, Amber = amber line, Green = green line
            if row['Days Overdue By'] > 0:
                bar_color = "#FF4B4B"  # Red for OVERDUE 100%
                bar_bg = "#5a1a1a"
            elif row['Days Overdue By'] >= -7:
                bar_color = "#FFA500"  # Amber for Due 7 days
                bar_bg = "#5a3d00"
            else:
                bar_color = "#00CC66"  # Green for OK
                bar_bg = "#0a3d1f"
            risk_pct = int(float(ai_risk) * 100)
            # Custom HTML progress bar with RAG color
            st.markdown(f'''
                <div style="background:{bar_bg}; border-radius:10px; height:22px; width:100%; margin:8px 0;">
                    <div style="background:{bar_color}; width:{risk_pct}%; height:22px; border-radius:10px; text-align:center; color:white; font-weight:bold; font-size:12px; line-height:22px;">
                        {risk_pct}%
                    </div>
                </div>
                <small>AI Risk: {ai_risk:.0%} - {row.get('AI_Status','')} - {row['RAG_Status']} | Bar color = RAG: {row['RAG_Emoji']} {row['RAG_Status']}</small>
        ...
        with col_d:
            if is_overdue or is_amber:
                if st.button(f"❌ Not Serviced", key=f"serv_{row['Asset Tag']}_{plant_type}_final_unique", use_container_width=True):
                    # FIXED: Keep REAL TIME and SAVE ORIGINAL DATE for Undo
                    now_dt = datetime.now()  # Real time e.g. 24/09/2026 10:12:42
                    now_for_file = pd.to_datetime(now_dt)
                    # SAVE ORIGINAL DATE BEFORE OVERWRITE FOR UNDO
                    original_last = row['Last Service']
                    if 'original_service_dates' not in st.session_state:
                        st.session_state.original_service_dates = {}
                    st.session_state.original_service_dates[row['Asset Tag']] = original_last
                        ...
                        try:
                            ...
                            if not verified_row.empty:
                                ...
                                st.session_state.last_file_updates[row['Asset Tag']] = {
                                    ...
                                    'written': now_dt.strftime("%d/%m/%Y %H:%M:%S"),
                                    'verified': str(verified_date),
                                    'mtime': mtime_str,
                                    'path': full_path,
                                    'original': str(original_last)
                                }
                                file_update_msg = f" | File: {file_name_label} updated at {mtime_str} - Verified: {verified_date} | Original was {original_last.strftime('%d/%m/%Y %H:%M:%S') if pd.notna(original_last) else 'N/A'}"
                                st.toast(f"Spreadsheet updated: {verified_date}", icon="✅")
                            else:
                                file_update_msg = f" | File {file_name_label} saved"
                        except Exception as e:
                            st.error(f"File update failed: {e} - Path: {full_path}")
                            file_update_msg = f" | ERROR: {e}"
                    else:
                        file_update_msg = " | No file path (upload mode)"
                        st.toast("Session updated", icon="⚠️")
                    
                    st.success(f"✅ {row['Asset Tag']} marked {now_dt.strftime('%d/%m/%Y %H:%M:%S')} - Now 🟢 GREEN!{file_update_msg}")
                    st.balloons()
                    st.rerun()
            else:
                # Serviced - GREEN state - SHOW UNDO THAT ACTUALLY RESTORES FILE
                ...
                if st.button(f"↩️ Undo - Mistake?", key=f"undo_{row['Asset Tag']}_{plant_type}_final_unique", use_container_width=True, help="Undo if mistakenly clicked - restores original date in file"):
                    # RESTORE ORIGINAL DATE TO FILE
                    restored_msg = ""
                    if full_path and row['Asset Tag'] in st.session_state.get('original_service_dates', {}):
                        try:
                            original_date = st.session_state.original_service_dates[row['Asset Tag']]
                            df_full = pd.read_excel(full_path, engine='openpyxl')
                            mask = df_full['Asset Tag'] == row['Asset Tag']
                            # Restore original
                            if pd.notna(original_date):
                                df_full.loc[mask, 'Last Service'] = pd.to_datetime(original_date)
                                if 'Date' in df_full.columns:
                                    df_full.loc[mask, 'Date'] = pd.to_datetime(original_date)
                            df_full.to_excel(full_path, index=False, engine='openpyxl')
                            restored_msg = f" | File restored to original: {original_date}"
                            st.toast(f"Undone! File restored to {original_date}", icon="↩️")
                        except Exception as e:
                            st.error(f"Undo file restore failed: {e}")
                            restored_msg = f" | Restore failed: {e}"
                    else:
                        restored_msg = " | No original date saved - session only"
                        st.toast("Undone - session cleared", icon="↩️")
                    
                    # Clear session states
                    if row['Asset Tag'] in st.session_state.serviced_assets:
                        del st.session_state.serviced_assets[row['Asset Tag']]
                    if 'original_service_dates' in st.session_state and row['Asset Tag'] in st.session_state.original_service_dates:
                        del st.session_state.original_service_dates[row['Asset Tag']]
                    ...
                    st.success(f"↩️ {row['Asset Tag']} undone! Restored to previous state{restored_msg}")
                    st.rerun()
