...
# Auto-restore from query params if session cleared on refresh
try:
    qp = st.query_params
...
if not st.session_state.authenticated:
    ...
    license_input = st.text_input("License Key:", type="password", placeholder="Enter license key", key="license_input_unique")
    if st.button("Unlock System", type="primary", use_container_width=True, key="unlock_system_btn"):
        result = check_license(license_input)
        if result["valid"]:
            st.session_state.authenticated = True
            st.session_state.client_info = result
            st.session_state.license_key = license_input.strip().upper()
            # Save to query params to survive refresh
            try:
...
# Keep license in query params for refresh survival
try:
...
if st.sidebar.button("Logout", key="logout_main_unique"):
    st.session_state.authenticated = False
    st.session_state.client_info = None
    try:
...
st.sidebar.header("🔄 Refresh - NO LOGOUT")
# Manual refresh that preserves login
if st.sidebar.button("🔄 Refresh Data (Stay Logged In)", use_container_width=True, key="manual_refresh_top_btn", help="Refreshes data without logging out"):
    st.cache_data.clear()
    st.rerun()

# Auto-refresh toggle - does NOT use JS reload, keeps session
auto_enabled = st.sidebar.toggle("Auto-refresh every 10 mins", value=False, key="auto_refresh_toggle_unique", help="Auto refresh without logout")
interval_mins = st.sidebar.slider("Interval (mins)", 1, 60, 10, key="refresh_interval_slider_unique")
if auto_enabled:
    import time
    ...
    if elapsed > interval_mins * 60:
        ...
        st.cache_data.clear()
        st.rerun()
    remaining = max(0, int(interval_mins*60 - elapsed))
    st.sidebar.caption(f"⏱️ Auto-refresh in {remaining}s - You stay logged in ✅")
    st.sidebar.caption("Uses st.rerun(), not browser reload")
else:
...
plant_type = st.sidebar.selectbox("Plant:", allowed_plants, index=0, key="plant_type_select_unique")
profile = PLANT_PROFILES[plant_type]

st.sidebar.header("Client Branding")
client_name = st.sidebar.text_input("Client Name", value=client_info["client"], key="client_name_input_unique")
plant_location = st.sidebar.text_input("Plant Location", value="Bonny Terminal", key="plant_location_input_unique")
prepared_by = st.sidebar.text_input("Prepared By", value=f"{client_info['client']} - Maintenance", key="prepared_by_input_unique")
logo_text = st.sidebar.text_input("Company Logo Text", value="MAINTAIN-AI", key="logo_text_input_unique")

st.sidebar.header("Data Source")
data_source = st.sidebar.radio("Excel Source:", ["Use Excel in Folder", "Upload Company Excel"], index=0, key="data_source_radio_unique")

if data_source == "Upload Company Excel":
    uploaded_file = st.sidebar.file_uploader("Upload Excel (.xlsx)", type=["xlsx","xls","csv"], key="excel_uploader_unique")
    ...
    if not xlsx_names:
        ...
        st.stop()
    selected = st.sidebar.selectbox("Log:", xlsx_names, index=0, key="log_selectbox_unique")
...
st.sidebar.header("Refresh - FIXED NO LOGOUT")
# FIXED: Manual refresh that does NOT log off - uses st.rerun() not window.reload()
if st.sidebar.button("🔄 Refresh Now (No Logout)", use_container_width=True, key="refresh_no_logout_btn"):
    st.rerun()

# FIXED: Auto-refresh using st.session_state timer, not JS reload that clears auth
auto_enabled = st.sidebar.toggle("Auto-refresh 10 mins (No Logout)", value=False)
interval_mins = st.sidebar.slider("Refresh mins", 1, 60, 10, key="refresh_interval_fixed")
if auto_enabled:
    # Use Streamlit's native rerun logic without clearing session_state
    import time
    if 'last_refresh' not in st.session_state:
        st.session_state.last_refresh = time.time()
    elapsed = time.time() - st.session_state.last_refresh
    if elapsed > interval_mins * 60:
        st.session_state.last_refresh = time.time()
        st.rerun()
    st.sidebar.caption(f"Next refresh in {int(interval_mins*60 - elapsed)}s - Auth kept")
